...
def ds_process_audio(ds, audio_file, file_handle, full_transcript_handle, vtt):
    ...
    if len(infered_text) != 0:
        line_count += 1
        write_to_file(file_handle, full_transcript_handle,
                      infered_text, line_count, limits, vtt, cues)
...
def main():
    ...
    for x in os.listdir(AUTOSUB_HOME):
        if x.endswith(".pbmm"):
            ...
            ds_model = os.path.join(AUTOSUB_HOME, x)
        if x.endswith(".scorer"):
            print("Scorer: ", os.path.join(AUTOSUB_HOME, x))
            ds_scorer = os.path.join(AUTOSUB_HOME, x)
    ...
    # full transcript
    full_transcript_name = os.path.join(
    ...
    for file in tqdm(sort_alphanumeric(os.listdir(audio_directory))):
        ...
        if audio_segment_path.split(os.sep)[-1] != audio_file_name.split(os.sep)[-1]:
            ds_process_audio(ds, audio_segment_path, file_handle,
                             full_transcript_handle, args.vtt)
